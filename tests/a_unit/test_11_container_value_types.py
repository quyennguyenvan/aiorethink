...
def test_tuple():
    ...
    assert vt.pyval_to_dbval(None) is None
    assert vt.dbval_to_pyval([1, 2, 3]) == (1, 2, 3)
    assert vt.dbval_to_pyval(None) is None
...
def test_namedtuple():
    ...
    for _vt in [vt, vt2]:
        ...
        assert _vt.pyval_to_dbval(None) is None
        assert _vt.dbval_to_pyval(v._asdict()) == v
        assert _vt.dbval_to_pyval(None) is None
